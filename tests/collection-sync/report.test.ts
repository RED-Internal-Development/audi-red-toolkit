...
import { collectCoverageReport } from "../../actions/collection-sync/src/collect.js";
import { buildCollectionReport } from "../../actions/collection-sync/src/report.js";

describe("collection-sync reporting", () => {
  ...
  test("parses Cypress text-summary output into the legacy report fields", async () => {
    ...
    const coverageRoot = await mkdtemp(join(tmpdir(), "collection-sync-nyc-"));
