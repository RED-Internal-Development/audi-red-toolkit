...
export async function syncDocs(inputs: DocsSyncInputs): Promise<boolean> {
  ...
  try {
    ...
    if (await hasRemoteBranch(cloneDir, inputs.destinationBranch)) {
      await runGit(
        clonePlan.checkoutExistingArgs,
        {
    ...
    await runGit(
      ["push", "-u", "origin", `HEAD:${inputs.destinationBranch}`],
      {
...
export interface ClonePlan {
  cloneArgs: string[];
  checkoutExistingArgs: string[];
  checkoutNewArgs: string[];
}
...
async function copySource(
  ...
  const destinationPath = inputs.rename
    ? join(destinationFolder, inputs.rename)
    : destinationFolder;
  ...
  if (inputs.useRsync) {
    await runCopyCommand(
      "rsync",
      ["-avrh", "--delete", inputs.sourceFile, destinationPath],
      "rsync failed while copying source path.",
    );
    return;
  }
  ...
  await cp(inputs.sourceFile, destinationPath, {
    ...
    throw new ActionError(
      "DOCSYNC_COPY_FAILED",
      "copy_source",
      "Failed copying source path to destination.",
    );
  });
}
...
async function hasRemoteBranch(cwd: string, branch: string): Promise<boolean> {
  ...
  throw new ActionError(
    ...
    `Failed checking whether destination branch '${branch}' exists after cloning.`,
  );
}
