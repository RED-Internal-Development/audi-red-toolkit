...
import type { MsiSyncInputs } from "./inputs.js";
import { type PublishPlan, type PublishPlanEntry } from "./plan.js";
import {
...
import { renderDirectoryTitleHtml, renderMarkdownToHtml } from "./render.js";
import { rewriteAttachmentReferences, stageMarkdownPage } from "./staging.js";
import { extractReferralId, PublishStats } from "./summary.js";

interface SyncRuntimeOptions {
  createClient?: (inputs: MsiSyncInputs) => ConfluenceClient;
}

export async function executeMsiSync(
  inputs: MsiSyncInputs,
  publishPlan: PublishPlan,
  stats: PublishStats,
  options: SyncRuntimeOptions = {},
): Promise<{ publishedPages: number }> {
  const stageRoot = await mkdtemp(join(tmpdir(), "msi-sync-stage-"));
  const client =
